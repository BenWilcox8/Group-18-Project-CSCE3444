...
function buildShareableURL(data: PlanData): string {
  const base = typeof window !== "undefined" ? window.location.origin : "";
  const params = new URLSearchParams();

  params.set("student", data.student);
  params.set("major", data.major);
  params.set("grad", data.expectedGraduation);
  params.set("total", String(data.totalCredits));
  params.set("completed", String(data.completedCredits));

  data.semesters.forEach((sem, i) => {
    params.set(`sem${i}`, sem.name);
    sem.courses.forEach((course, j) => {
      params.set(`s${i}c${j}`, `${course.code}|${course.name}|${course.credits}`);
    });
  });

  return `${base}/plan?${params.toString()}`;
}
...
export function ExportPlan({ planData }: ExportPlanProps) {
  ...
  useEffect(() => {
    setShareableURL(buildShareableURL(data));
  }, [data]);
  ...
  const handleExport = (formatId: string) => {
    ...
    setTimeout(() => {
      if (formatId === "csv") {
        downloadFile(generateCSV(), "text/csv;charset=utf-8;", "eagle_plan.csv");
      } else if (formatId === "ical") {
        downloadFile(generateICS(), "text/calendar;charset=utf-8;", "eagle_plan_calendar.ics");
      }
      ...
      <CardContent className="space-y-6">
        ...
        <div className="p-4 bg-primary/5 rounded-xl border border-primary/20">
          <h4 className="font-semibold text-foreground mb-2 flex items-center gap-2">
            <ExternalLink className="h-4 w-4 text-primary" aria-hidden="true" /> Share with Advisor
          </h4>
          <p className="text-sm text-muted-foreground mb-3">
            Copy this link and send it to your advisor — it opens a clean view of your full schedule.
          </p>
          <div className="flex gap-2">
            <div
              className="flex-1 p-3 bg-background rounded-lg border border-border text-sm text-muted-foreground truncate"
              title={shareableURL}
            >
              {shareableURL || "Add courses to generate a link"}
            </div>
            <Button variant="outline" onClick={handleCopyLink} disabled={!shareableURL}>
              {copiedLink
